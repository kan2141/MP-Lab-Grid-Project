...
VolDF = pd.read_excel('Data.xlsx', sheet_name='Vol', index_col=0)
ThetaDF = pd.read_excel('Data.xlsx', sheet_name='Theta', index_col=0)
print(ThetaDF)
...
VolDict = { (t,c) : VolDF.to_dict('index')[t][c] for t in VolDF.index for c in VolDF.columns }
ThetaDict = { (t,c) : ThetaDF.to_dict('index')[t][c] for t in ThetaDF.index for c in ThetaDF.columns }

# Solve OPF
opf.opf(PdDict, QdDict, PpvDict, QpvDict, PwtDict, QwtDict, PG2Dict, QG2Dict, PC1Dict, QC1Dict, VolDict, ThetaDict)